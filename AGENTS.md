...
This is a QMK userspace repository for building Vial-compatible firmware for the Lily58 keyboard using the Helios MCU conversion. The project is configured as an external QMK userspace that builds against [vial-kb/vial-qmk](https://github.com/vial-kb/vial-qmk) rather than mainline QMK. The base repo is tracked as the `qmk_firmware` submodule at the root of this userspace.
The documentation for qmk can be found at https://docs.qmk.fm/.

Follow British spelling conventions (e.g., "customise", "capitalise", "behaviour").
...
- **mise**: Task runner and tool manager — installs QMK CLI, clangd, and defines all build tasks in `mise.toml`
- **GitHub Actions**: Uses `vial-kb/vial-qmk` fork instead of mainline QMK
- **VS Code tasks**: Thin wrappers around mise tasks (defined in `.vscode/tasks.json`)

## Development Workflows

### mise Tasks

All build commands are defined in `mise.toml` and run via `mise run <task>`:

- **`mise run build`**: Compile Lily58 firmware (primary build command)
- **`mise run compile`**: Compile and generate `compile_commands.json` for clangd
- **`mise run flash`**: Compile and flash firmware to keyboard
- **`mise run generate-autocorrect`**: Regenerate `autocorrect_data.h` from dictionary file
- **`mise run update-firmware`**: Update the `qmk_firmware` submodule and sync submodules

VS Code tasks (`Ctrl+Shift+B` for default build) delegate to these same mise tasks.
